...
c*********************************************************
      subroutine testprof(b0o,ai,b0,ntae,tip2,vi,zi)
      include 'clich1'
      include 'clich1b'
      include 'clich2'
      common/functn/omreal,eigfun(nn,mt,3),eigfun_s(nn,mt,3)
      common/funct/xkpop(nn,mt,nts,3)
      dimension dencc(nn,isph),denhh(nn,isph),vstar(nn),xnnn(nn),
     &     grps2(1000),rgridc(1000)
      parameter (nz1=10)
      character col(nz1),pat(nz1),lbl*40,tip2*1
      external fwd
      complex wtae
      tip=tip2
      do i=1,nz1
      ...
      vmax=1.07
      wtae=0.1
      call t0xhp(alpha,t0,xhp,ai,b0,b0o,vi,zi,psitot)
      call eigenmod(b0o,b0,dbobmax,dbobmaxs,dnonmax,grps2(1),rgridc(1),
     &     min,min_f,max,max_f,ntae,1.,valfv,wtae,deltk,ximax)
      omstar=real(ntae)*xhp*vi*10./real(wtae)
      write(*,*) 'omstar,xhp,wtae,tip',omstar,xhp,wtae,tip
c      write(*,*) ' type t for test or s for sl.down'
c      read(*,'(a)')tip
c      write(*,*) rgrid,denc
      xv=0.5
      do j=1,nosurf,151
         do i=1,nosurf
            ...
            omstar=real(ntae)*xhp*vi*10./real(wtae)
            psiavres=1.
cterm introduce hhh1 here
            call distrfun(4.,b0o*b0,dfalp,dencc(i,j),denhh(i,j),dpga,1.e
     &          -5,psiavres,1.,hnub,h2,3,0,.1,p0ga,1.,1.,1.,tb,tip,xv,vi
     &           ,vmax,0.,0.,xhp,zi,dwbdpphi,wb_res)
c            dencc(i,j)=abs(dencc(i,j))/1.e10
            dencc(i,j)=1.+omstar*xv*denhh(i,j)/dencc(i,j)
            write(*,*) psiavres,dfalp,dencc(i,j),denhh(i,j)
            denhh(i,j)=abs(denhh(i,j))/1.e10/amax1(dencc(i,j),1.)
            delk(i)=abs(delk(i))
            rhoprf(i)=abs(rhoprf(i))
            xnnn(i)=1/zi
         enddo
      enddo
      write(*,*) 'rho0= ',rho0,' psit.= ',psitot
c      write(*,*) (dencc(i,1),i=1,nosurf)
c      write(*,*) (denhh(i,1),i=1,nosurf)
c      write(*,*) (ppsi(i),i=1,nosurf)
      call twodgraf(xnnn(1),dencc(1,1),nosurf,1,col(1),pat(1),'white'
     $     ,2,'1-om_*/om','1/Z_alpha','1-om_*/om','2d',6,6,1,dencc(1,1)
     &     ,0)
      write(lbl,'(a,e14.3)') ' Psi_av=',psiavres
      call plchhq(.3,.15,lbl,0.01, 0.,0.)
      call frame(0)
c      call twodgraf(xnnn,rgrid,nosurf,1,col(1),pat(1),'white'
c     $     ,2,'hot','sqrt(Psi)','Density,cm-3','2d',6,6,1,denhh(1,1),0)
c      call frame(0)
c      call twodgraf(rgrid,ppsi,nosurf,1,col(1),pat(1),'white'
c     $     ,2,'hot','sqrt(Psi)','Density,cm-3','2d',6,6,1,denhh(1,1),0)
c      call frame(0)
c      call twodgraf(rgrid,denhh(1,1),nosurf,1,col(1),pat(1),'white'
c     $     ,2,'hot','sqrt(Psi)','Density,cm-3','2d',6,6,1,denhh(1,1),0)
c      call frame(0)
c      return
c      call twodgraf(rgrid,betaa,nosurf,1,col(1),pat(1),'white'
c     $     ,2,'alpha','sqrt(Psi)','Beta','2d',6,6,1,betaa,0)
c      call frame(0)
c      call twodgraf(rgrid(4),delk(4),nosurf-3,1,col(1),pat(1),'white'
c     $     ,2,'alpha','sqrt(Psi)','Delta kinet.','2d',6,6,1,betaa,0)
c      call frame(0)
c      call twodgraf(rgrid,rhoprf,nosurf,1,col(1),pat(1),'white'
c     $     ,2,'alpha','sqrt(Psi)','Beta','2d',6,6,1,betaa,0)
c      call frame(0)
c      call twodgraf(rgrid,vstar,nosurf,1,col(1),pat(1),'white'
c     $     ,2,'alpha','sqrt(Psi)','vstar','2d',6,6,1,vstar,0)
c      call frame(0)
c      call twodgraf(rgrid,tc(1,1),nosurf,5,col(1),pat(1),'white'
c     $     ,2,'core','sqrt(Psi)','T_c,eV','2d',6,6,1,tc,0)
c      call frame(0)
      return
      end
c*********************************************************
...
c
c               write(*,*) '1htb,pss,dzpsi,r,a,b  ',htb,pss,dzpsi,r,a
c     &              ,b
               htb=max(abs(htb),.003)
               htb=r**2/htb*bm
            else
...
c
c               write(*,*) '2htb,pss,dzpsi,r,a,b  ',htb,pss,dzpsi,r,a
c     &              ,b
               htb=max(abs(htb),.003)
               htb=r**2/htb*bm
            endif
...
c values for NSTX-U devices are left according to M. Gorelenkova: xfull/0.461/, xhalf/0.386/
      ...
      save tip1,vstar3,vstar3s,cvsld,denshh,cv,dnpsi0,betalpha0,cnorm,sa
      tip=tip2
c This shortcut is done to simplify the coding, which is not good since
c it calculates the normalization every time. However this workaround is
c needed since the normalization depends on the velocity, which changes
c every call. 
...
ckg b0oo*3. byl vveden (ran'she ne bylo (bug))
...
c         endif
         ...
         tau3=-alog(abs(vinst3**3/vstar3)*(1.+p3xv3))*0.33333333333
c
...
c summ all the DF from 3 sources depending on the ion energy
     ...
     &        vstar3)+dfalp3*ddflpdgc3*(1.-p0ga*p0ga)/(vinst3*p0ga))
...
c      see above definition for xfull xhalf
      if(tip.eq.'b')then
         dfalpdv=dfalpdv*xfull
         dfalp=dfalp*xfull
         dfalpdpsi=dfalpdpsi*xfull
         if(vinstant1.lt.0.7071*vmax)then
            dfalpdv=dfalpdv+dfalpdv2*xhalf
            dfalp=dfalp+dfalp2*xhalf
            dfalpdpsi=dfalpdpsi+dfalpdpsi2*xhalf
            if(vinstant1.lt.0.57735*vmax)then
               dfalpdv=dfalpdv+dfalpdv3*(1-xfull-xhalf)
               dfalp=dfalp+dfalp3*(1-xfull-xhalf)
               dfalpdpsi=dfalpdpsi+dfalpdpsi3*(1-xfull-xhalf)
            endif
         endif
      endif
c      dfalpdv=0.
...
c this part includes both radial and poloidal
c
                  xkpop(i,im,jts,i3)=sqrt(grthsq2d(i,jts)*m2+dyn
c
     &                 *grpssq2d(i,jts))
               enddo
            enddo
