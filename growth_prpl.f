...
c calculation of the principal part of the dW_k
     ...
     &     ,dpga,dintgr,fo,hnub,hnb,hnub_se,hwb1,mt,mtl,nnres,eps,
...
c use this to substitute for pause 
...
ckg_sat amplitude saturation stuff 
      dimension 
c      common/rbq_sat/??? may give some errors during the compilation???
     ...
     &     condec(1000),condic(1000),denc(1000,3),
...
c_________graph\/staff
      ...
      complex wtae
      data omkomis/2*0./,gamkomis/12*0./,gtrap1/6*0./,idlt/9/ ! idlt's comments see below 
      nmuc=150
      npfc=200
...
c========== definitions for integrations
      ...
      call eigenmod(b0oo,b0,condec(1),condic(1),denc(1,1),
     &     dbobmax,dbobmaxs,dnonmax,grps2(1),rgrid(1),
     &     min,min_f,max,max_f,ntae,scalvavf,valfv,
     &     wtae,deltk,ximax)
     ...
     &     ,psiav,psitot,psma,psmi,pmu,nmuc,qpf,npfc,r0,rf,rma
...
c  f~/(v^3 + v*^3)^(2-gam) and gam=1 corresponds to the slowing down df.
     ...
     &     /b0oo/b0/4./deltk/r0
#ifdef nbiE0xtra
      gamkom=gamkom*(nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.))**1.5     ! 1.5 accounts 4 xhp
#endif
      gamkomhlp=gamkom
      omstari=real(ntae)*xhp*vi*10.
#ifdef nbiE0xtra
      omstari=omstari*nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.)
#endif
...
c      if(im1.ne.0) then   ! include rotation profile from transp.dat file
      if(im1.ne.1) then   ! not to include rotation profile
c         wtae=(0.,0.)
         w_rot0=0.
         w_rot1=0.
...
c rotation frequencies are read in my units 10^6 rad/sec^-1
     ...
     &     ,' w_rot1=',w_rot1,' w_rotres=',w_rotres
      ...
      if(ai.eq.4..and.tip.ne.'i')then      ! Alphas
         filename='gam'//fow(1:1)//fow(3:4)//'.'//fow(5:5)//fow(7:8)/
     &        /'a'
         ihsps=3
      else if(ai.eq.1..and.tip.ne.'i')then ! Hydrogen
         filename='gam'//fow(1:1)//fow(3:4)//'.'//fow(5:5)//fow(7:8)/
     &        /'h'
         ihsps=3
      else if(ai.eq.2..and.tip.ne.'i')then ! Deuterium, good for thermal ion Landau damp.
         filename='gam'//fow(1:1)//fow(3:4)//'.'//fow(5:5)//fow(7:8)/
     &        /'d'
         ihsps=1
      else if(ai.eq.3..and.zi.eq.2.and.tip.eq.'h')then ! He3
         filename='gam'//fow(1:1)//fow(3:4)//'.'//fow(5:5)//fow(7:8)/
     &        /'he3'
         ihsps=3
      else if(ai.eq.3.)then                            ! Tritium, good for thermal ion Landau damp.
         filename='gam'//fow(1:1)//fow(3:4)//'.'//fow(5:5)//fow(7:8)/
     &        /'t'
         ihsps=2
...
c         symm=1.
         is1t=1
         is2t=is1t
c         ipf1t=indx(qpf(1),npfc,1.)
c at 100,50 runs OK if the broadened region is gdp2pf=0.1 inside fungw0 
         ipf1t=1
         ipf2t=npfc
         ipf1t=33                ! for work/nova/iter_isep21/iter_131041steadystateHResol test runs
         ipf2t=33                ! for work/rbq/d3d_176523oscill/t0615
c         ipf2t=ipf1t
c mu limits
         imu1t=1                 !8 1   -> 0
         imu2t=nmuc              !8 nmu -> R0+a
         imu1t=106                ! for work/nova/iter_isep21/iter_131041steadystateHResol test runs
         imu2t=106                ! for work/rbq/d3d_176523oscill/t0615
c         imu2t=imu1t+1           ! +1 m.b. enough extra point to control the calculations
...
c no extra output if =30 just run with limited l numbers
         itestil=10   ! 0 is default, i.e. when no choice is made for harmonics
         il1t=-mtl
         il2t=mtl
         il1t=-4
         il2t=-2
         il1t=-20
         il2t=20
                                ! define here il{1,2}t for itestil=1,10,30
c         il1t=-23
c         il2t=-21
c         il1t=-7                ! for work/rbq/d3d_176523oscill/t0615
c         il2t=-5
c
c         il2t=il1t+4
c To test for each mode harmonics:
         itestm=1               ! either 1 or 0; deeper loop for itest=1
...
c poloidal numbers, which differs from Max M value printed!!!!
         im1t=min_f
         im2t=max_f
c         im1t=5    ! for work/rbq/d3d_176523oscill/t0615
c         im2t=7
      else
         im1t=min_f
         im2t=max_f
...
#ifdef nbiE0xtra
      ivmax=indx(v,nv,
     &     sqrt(nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.))+0.07)
      print *,'ivmax=',ivmax, indx(v,nv,
     &     sqrt(nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.))), 
     &     'nbiE0xtra',nbiE0xtra,'ai=',ai,'vi=',vi
#endif
      irept_sat=1
#ifdef _dev
      nrept=2
      print *,'Going to _dev loop with nrept=2'
c      nrept=1
c22 this repetition is introduced to account for only important sidebands for averaging of of nu_scatt
      do irept_sat=1,nrept
#endif
      do is=is1t,is2t
         sigm=1.
         if(is.eq.2) sigm=-1.
ctem
         if(im1.eq.1.and.itest.ne.1)ipf2t=npfc*3/4
c         if(tip.eq.'r')then
c            ipf1t=npfc*3/7.
c            ipf2t=npfc*5/7.
c         endif
...
c
            deltqx=sigm*(qpf(npfc)-qpf(1)-1.)/2.
            ilmax=-1000
            ilmin=1000
            imu_m1=0
            do imu=imu1t,imu2t
...
c
               ...
               ilmax=-1000
               ilmin=1000
cnng21 should be avoided                   hnub=1.e-3
               hnb=0.
               hnub=0.
               do imtae=min1,max1
                  imtae0=imtae+1-min
...
#ifdef _dev
c this statement chooses 1 of two resonances for smooth representation in RBQ, see also below
                           if(is.eq.1) then
...
c     Psi_edge and are in cm^-3
     ...
     &                          ,psiavres(ires,il),gam,hnubiresil,  !coll.scatt.freq. in 10^6 sec^-1
     &             hchirp(1,ires,il),ihsps,iter,qpfi,p0ga,pmui,rvpar   !hchirp(2,,) drag sl.d.freq.in sec^-1; (1,,) coll.scat. in sec^-1
#ifdef nbiE0xtra
     &                         ,sigm,tb,tip,vres(ires,il),vi*sqrt(
     &          nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.)),v(ivmax),
#else
     &                         ,sigm,tb,tip,vres(ires,il),vi,v(nv),
#endif
     &                          0.,z0,xhp,zi,dwbdpphi,wb_res,istart)
ckg_sat
                           hnub(ires,il)=hnubiresil
                           hnb(ires,il)=hnubiresil
                           if(irept_sat.eq.1.or.
     &                       (irept_sat.eq.2.and.il.ge.indx_sat(imu,is)
     &                       .and.il.le.indx_sat(imu,is)+2*idlt))then
                              hnb(ires,il)=hnubiresil
                           else
                              hnb(ires,il)=0.
                           endif
...
c:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
               if(im1.ne.0.and.im1.ne.3)then
                  if(pk.ne.1.) stop 'pk.ne.1.'   ! for m1 stability option
                  do ivm1=1,nv
...
#ifdef nbiE0xtra
c     &                         ,sigm,tb,tip,vres(ires,il),vi*sqrt(
c     &          nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.)),v(ivmax),
     &                       ,sigm,tb,tip,v(ivm1),vi*sqrt(
     &          nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.)),v(ivmax),0.
#else
     &                       ,sigm,tb,tip,v(ivm1),vi,v(nv),0.
#endif
...
cnng24                           if(itestm.eq.1) print *,'fos',fo(1,imtae,il,
c     & ires),fo(2,imtae,il,ires),fo(3,imtae,il,ires),fo(4,imtae,il,ires)
                        enddo
...
c                            /u/Ql../Ql../ChirpStabXtendExt.pdf to evaluate chirping criterion
                           do idrg=1,iberk
                              do isct=1,iberk
...
cnng21
     &                     hhb*hnb(ires,il)*1.e+18
c     &                     hhb*hnub(ires,il)*1.e+18
...
c                        dfdfdvdr(ipf,imu,il,ires,is)=dfalpdv(ires,il)
     ...
     &                ,hhh2,dfalpdv(ires,il),dfalp(ires,il)
                        hhh11=hhh1
ckg_sat
...
c     passing w FLR
     ...
     &                          ,ml2
                           write(*,*) 'With FLR',hhh2,' W/o FLR' !for131041ss il-22indx-31is1ires2imu25ipf24 indx m.b. determined from rbq run
     &     ,hhh11,'idx',il+31+(idlt*2+1)
     &     *(is-1+2*(imu-1+nmuc*(ipf-1+npfc*0))),il,is,ires,imu,ipf,
     &     indx_sat(imu,is),nmuc,'vgam',
     &     vgam_sat(ipf,imu,il,ires,is),vres_sat(ipf,imu,il,1,is),
     & vres_sat(ipf,imu,il,2,is)
                        endif
...
c /\ end of MU - pitch angle integration loop
            call asimp(pmu,gamintmu(1,1),nmuc,gamintpfi(ipf,1,is))
            call asimp(pmu,gamintmu(1,2),nmuc,gamintpfi(ipf,2,is))
            do ib=1,iberk
               call asimp(pmu,gberkintmu(1,ib,1),nmuc,gberkintpfi(ipf
     &              ,ib,2*is-1))
               call asimp(pmu,gberkintmu(1,ib,2),nmuc,gberkintpfi(ipf
     &              ,ib,2*is))
               call asimp(pmu,gberkintmu(1,ib,3),nmuc,gberkintpfi(ipf
     &              ,ib,4+is))
               call asimp(pmu,gberkintmu(1,ib,4),nmuc,gberkintpfi(ipf
     &              ,ib,6+is))
            enddo
            call asimp(pmu,gberkimuch(1,1),nmuc,gberkipfich(ipf,1,is))
            call asimp(pmu,gberkimuch(1,2),nmuc,gberkipfich(ipf,2,is))
            do idrg=1,iberk
               do isct=1,iberk
                  call asimp(pmu,gbimuch(1,idrg,isct),nmuc,gbipfich(
     &                 ipf,idrg,isct,is))
               enddo
            enddo
            call asimp(pmu,gbimuch0(1),nmuc,gbipfich0(ipf,is))
            call asimp(pmu,gbidrgavmu(1,1),nmuc,gbidrgavipfi(ipf,is,1))
            call asimp(pmu,gbidrgavmu(1,2),nmuc,gbidrgavipfi(ipf,is,2))
            call asimp(pmu,gbidrgavmu(1,3),nmuc,gbidrgavipfi(ipf,is,3))
            call asimp(pmu,gbidrgavmu(1,4),nmuc,gbidrgavipfi(ipf,is,4))
            if(is.eq.1) then
               call asimp(pmu,gamintmu(1,3),nmuc,gtrap(ipf,1))
               call asimp(pmu,gamintmu(1,4),nmuc,gtrap(ipf,2))
               call asimp(pmu,gamintmu(1,7),nmuc,gtrap(ipf,5))
               call asimp(pmu,gamintmu(1,8),nmuc,gtrap(ipf,6))
            endif
            call asimp(pmu,omintmu(1,1),nmuc,hhh1)
            omintpfi(ipf,1)=omintpfi(ipf,1)+hhh1
            call asimp(pmu,omintmu(1,2),nmuc,hhh2)
            omintpfi(ipf,2)=omintpfi(ipf,2)+hhh2
c            write(*,*) 'Here is gamu',(gamintmu(i,5),i=1,nmuc)
c            write(*,*) '----'
            call asimp(pmu,gamintmu(1,5),nmuc,gamintpfi(ipf,5,is))
            call asimp(pmu,gamintmu(1,6),nmuc,gamintpfi(ipf,6,is))
...
c     &           ,gamintpfi(ipf,3,2)*gamkom,gtrap(ipf,2)*gamkom
            if(ipf.eq.npfc.or.mod(ipf,3).eq.0)then
...
c /\ end of integration in minor radius, i.e. P_phi
         ...
         do ib=1,iberk
            call asimp(qpf,gberkintpfi(1,ib,2*is-1),npfc,gberkis(ib,2*is
     &           -1))
            call asimp(qpf,gberkintpfi(1,ib,2*is),npfc,gberkis(ib,2*is))
            call asimp(qpf,gberkintpfi(1,ib,4+is),npfc,gberkis(ib,4+is))
            call asimp(qpf,gberkintpfi(1,ib,6+is),npfc,gberkis(ib,6+is))
         enddo
         call asimp(qpf,gberkipfich(1,1,is),npfc,gberkisch(1,is))
         call asimp(qpf,gberkipfich(1,2,is),npfc,gberkisch(2,is))
         do idrg=1,iberk
            do isct=1,iberk
               call asimp(qpf,gbipfich(1,idrg,isct,is),npfc,gbisch
     &              (idrg,isct,is))
            enddo
         enddo
         call asimp(qpf,gbipfich0(1,is),npfc,gbisch0(is))
         call asimp(qpf,gbidrgavipfi(1,is,1),npfc,gbidrgavis(is,1))
         call asimp(qpf,gbidrgavipfi(1,is,2),npfc,gbidrgavis(is,2))
         call asimp(qpf,gbidrgavipfi(1,is,3),npfc,gbidrgavis(is,3))
         call asimp(qpf,gbidrgavipfi(1,is,4),npfc,gbidrgavis(is,4))
c
         call asimp(qpf,gamintpfi(1,1,is),npfc,gamkomis(1,is))
         call asimp(qpf,gamintpfi(1,2,is),npfc,gamkomis(2,is))
         call asimp(qpf,gamintpfi(1,3,is),npfc,gamkomis(3,is))
         gamkomis(3,is)=gamkomis(3,is)*gamkom
         call asimp(qpf,gamintpfi(1,4,is),npfc,gamkomis(4,is))
         gamkomis(4,is)=gamkomis(4,is)*gamkom
         call asimp(qpf,gamintpfi(1,5,is),npfc,gamkomis(5,is))
         call asimp(qpf,gamintpfi(1,6,is),npfc,gamkomis(6,is))
         call asimp(qpf,omintpfi(1,1),npfc,hhh1)
         omkomis(1)=omkomis(1)+hhh1
         call asimp(qpf,omintpfi(1,2),npfc,hhh2)
         omkomis(2)=omkomis(2)+hhh2
         if(is.eq.1) then
            call asimp(qpf,gtrap(1,1),npfc,gtrap1(1))
            call asimp(qpf,gtrap(1,2),npfc,gtrap1(2))
            call asimp(qpf,gtrap(1,3),npfc,gtrap1(3))
            gtrap1(3)=gtrap1(3)*gamkom
            call asimp(qpf,gtrap(1,4),npfc,gtrap1(4))
            gtrap1(4)=gtrap1(4)*gamkom
            call asimp(qpf,gtrap(1,5),npfc,gtrap1(5))
            call asimp(qpf,gtrap(1,6),npfc,gtrap1(6))
         endif
      enddo
...
c code does it autmatically.
      call system("rm -f buf3")
      c46=
     &"ls -l eigenf|awk '{print $NF}'>buf3"
      call system(c46)
      open(11,file='buf3',status='old')
      read(11,*) c46
...
c modify the file to add other information: gdTot and gamkomT (or with/w/o FLR gamkomflr/gamkomT)
     ...
     &     gamkomflr,gdTot-gcontL,ximax,dnonmax
c change its contents to add frequencies and growth rates
         call system("sed s/'6 ;S'/'11 ;S'/ "//c46//" > buf3")
         call system
     &   ("sed s/' ;N'/'"//cgw(1:60)//" ;N'/ buf3 > buf31")
         call system(
     &      "sed s/'igrid;'/'igrid,gw[kHz],gwkwFlr,gdTot;'/
     &      buf31 > buf3")
c here careful check should be done for gdTot summation - 
c all terms or not are included? grL=RadDamp is include, but no gcontL
         call system(
     &"sed s/'gdTot;'/'gdTot=grad+gCL+gHL+geL+gecoll+gTL+grL;'/
     &buf3> buf31")
c
         write(*,*) c46
c        call system("sed s/'gTL;'/'gTL+gDL,Xi[m],dn/n;'/
     ...
     &     buf31> k"//c46)
cremoving buf3 files not to interfair with other users
         call system("rm -f buf3")
         call system("rm -f buf31")
      else
         write(*,*) 
...
c change of plans: do this for the whole resonance, not at specific ipf
      ires=1
c next lines not needed, moved to the top data defntx
c    idlt=4 ! this is for 2*4+1 = 9 harmonics to be accounted for in RBQ analysis 
c      idlt=3 ! this is smaller but sufficient to get 7 harmonics for RBQ runs: nlc=7
c      idlt=9 ! this is for => nlc = 2*9+1 = 19
c      idlt=17 ! take this for tests
c      idlt=12 ! this is for => nlc = 2*12+1 = 25
c      idlt=17 is what is used for 176523 with nlc = 35 or saved as In/gxmn03w.6474E+01.u4mgxnonabs
      i1D=1                     !this is equidistant in velocity, which is for 1D case
      if(pk==0.5) then
         i1D=2                  !this is equidistant in energy, which is for 2D case
      endif
      c46(2:2)='m'
      if(ai.eq.2.) c46(2:2)='b'
      open(unit=18,file='gx'//c46(2:15)//'.u4m',status='unknown')
     ...
     & trim(adjustl(runid(it:90))),'; mode ',c46(3:15),';'
      ...
      write(18,*) 0,23,';Sclrs;Nsclrs;' !0 for scalars (1st array); 23 for number of scalars
      write(18,*) npfc,nmuc,mtl_m1,2,ntae,real(wtae),gamkom,          !  7 consts
#ifdef nbiE0xtra
     &    idlt*2+1,vi*sqrt(nbiE0xtra*1.298**2*2./(ai*vi**2*880.*2.))
     &    ,ai,zi,omstar,i1D,xhp,psitot,r0,valfv*10./r0,    !+10 consts
#else
     &    idlt*2+1,vi,ai,zi,omstar,i1D,xhp,psitot,r0,valfv*10./r0,    !+10 consts
#endif
     &     gdTot-gcontL,gamkomflr,ximax,dbobmax,min,pk,           !+ 6 consts !only cont damp excluded
     &     ';npfc,nmuc,mtl_m1,2,ntae',
     &     ',wtae[10^6/sec],gamkom,idlt2+1,vi,ai,zi,omstar,i1D,xhp',
     &  ',psitot,rax,walfv[10^6/sec],gdTot,gghNflr,Xi[m],dB/B,minm,pk;' !first sequence/array of scalars
cnng16 
      do imu=1,nmuc
c reuse aeqv array to find the dominant resonances
         do is=1,2
            ...
            do ipf=1,npfc
               do il=-mtl,mtl
c identfy doubl resonant location and put on the first place,i.e. resonance at largest resont velocity
                  if(
! vres_sat(ipf,imu,il,ires,is).gt.0.0.and.
     & vres_sat(ipf,imu,il,2,is).gt.0.and.abs(vgam_sat(ipf,imu,il,2,is))
     & .gt.abs(vgam_sat(ipf,imu,il,1,is))) then
! redefining to ires=1 values from ires=2
...
cnng21                  hhhgam=abs(gam_sat(ipf,imu,il,ires,is))
cnng22
                  hhhgam=gam_sat(ipf,imu,il,ires,is)
                  if(abs(vres_sat(ipf,imu,il,ires+1,is)).gt.0.001)
     &                   hhhgam=hhhgam+gam_sat(ipf,imu,il,ires+1,is)
                  aeqv(1,il)=aeqv(1,il)+hhhgam
                  if(abs(hhhmax).lt.abs(hhhgam)) then
                     hhhmax=hhhgam
                     ilmax=il
                     ipfmx=ipf
                  endif
...
c after integration check the maximum harmonic contribution
            ...
            if(itest.eq.1) print *,'ihhh',ihhh,'hhh',hhh,'imu',imu,
     &           'is',is,'ilmax',ilmax,'ipfmx',ipfmx,'hhhmax',hhhmax
c            vres_sat(ipf,imu,il,ires,is)=vres_sat(ipf,imu,il,ires,is)
c
...
c     define array for minimization of Taylor expansion of the Omega
...
c         this loop is entered each time for each harmonics and if there is a resonance
               do ipf=1,npfc
                if((vres_sat(ipf,imu,il,ires,is).gt.0.001).and.(ipf.lt
     &         .npfc).and.(vres_sat(ipf+1,imu,il,ires,is).gt.0.001))then
                     ifail=0
#ifdef _test1
                        write(*,'(a,2e12.4,a,2e12.4,a,e12.4,4i4)')
     ^                    'B',vres_sat(ipf,imu,il,ires,is),qpf(ipf),'('
     ^                       ,dwp_sat(ipf,imu,il,ires,is)
     ...
     ^                       ,d2wpmnmz_sat(ipf,imu,il,ires,is),nmuc,imu
     ^                      ,npfc,nv,qpf,v ,qpf(ipf),vres_sat(ipf,imu,il
     ^                       ,ires,is),  0.  ,wb(1,1,1,is),ifail)
                     else
c                        if(ipf.eq.5.and.is.eq.1) pause
                        call wresminimize(wresPfiV(1,1),dwp_sat(ipf,imu,
     ^                       il,ires,is),dwpmnmz_sat(ipf,imu,il,ires,is)
     ^                       ,d2wpmnmz_sat(ipf,imu,il,ires,is),nmuc,imu
     ^                      ,npfc,nv,qpf,v ,qpf(ipf),vres_sat(ipf,imu,il
...
#ifdef _test1
                        write(*,'(a,2e12.4,a,2e12.4,a,e12.4,4i4)')
     ^                    'A',vres_sat(ipf,imu,il,ires,is),qpf(ipf),'('
     ^                       ,dwp_sat(ipf,imu,il,ires,is)
     ^                       ,dwpmnmz_sat(ipf,imu,il,ires,is),')'
     ^                       ,d2wpmnmz_sat(ipf,imu,il,ires,is),ipf ,imu
     ^                       ,il,is
      ...
      enddo                     ! is loop for mu irept_sat=1,nrept
      if(nrept==2.and.irept_sat.eq.1) close(18)
      enddo ! irept_sat loop
c
      ...
      write(18,*) 2,2,'; indx_sat; eigfun; vs nmuc,2 and vs nn,mt;'
      write(18,*) nmuc*2,'; indx;'
      write(18,*) indx_sat      !second sequence
c saving the radial displacement for RBQ through a separate subroutine 
      call savexi(18) 
c if 5,13 numbers are changed make similar changes in RBQ 
      write(18,*)  5,13,'; dwv_sat; vres_sat; fdist_sat; gam_sat; ',
     &'vgam_sat; tbres_sat; dwp_sat; dwpmnmz_sat; d2wpmnmz_sat; ',
     &'hnub_sat; bgy_sat; wbwpi_sat; meigfun; vs npf,nmuc,idlt*2+1,1,2;'
      open(unit=19,file='buf.dat',status='unknown')
      is=1
      do ipf3=1,npfc
      do imu3=1,nmuc
      do is3=1,2
      do il3=indx_sat(imu3,is3),indx_sat(imu3,is3)+2*idlt
      do ires3=1,2
      write(19,'(e18.12,5i4,i9)')vgam_sat(ipf3,imu3,il3,
     & ires3,is3),ipf3,imu3,il3,ires3,is3,il3+31+(idlt*2+1)
     &     *(is3-1+2*(imu3-1+nmuc*(ipf3-1+npfc*0)))
      enddo
      enddo
      enddo
      enddo
...
c     this value corresponds roughly to half input energy of hot ions if
            ivc=nv/v(nv)*0.7
c but we have to choose it equal to the Alfven speed; this is typical for alpha or NBI ions in ITER
...
c     otherwise be  close to the latest one
         if(ivc.ge.nv) then
            ...
            ivc=min0(nv,ivc)
            ivc=max0(1,ivc)
cnng24 we presribe ivc to most interesting energy which could be something close to injection energy for DIIID
            ivc=nv/v(nv)        ! that will bring ivc to injection velocity value
         endif
         labtitle='distr.function'
c         open(unit=18,file='df_type_'//tip(1:1)//'.bdat',form=
c     &        'unformatted',status='unknown') ! make sure the same "unformated" is in reading routine
         open(unit=18,file='df_type_'//tip(1:1)//'.bdat',
     &        status='unknown')
         imuax=indx(pmu(1),nmuc,r0)
c         write(*,*) '*** in growth imuax=',imuax
c choose Pphi index at R_LFS corresponding to the maximum of mode amplitude
         ratmax=ratpsii(isrfmax,psii,bii,bif)  ! isrfmax is the tip of max AE amplitude; bii is B; psii is Psi at that location
         ipfmax=indx(qpf,npfc,psii)+5         ! indx of Pfi at max ampltd
         do iv=1,nv
            do ipf=1,npfc
               do imu=1,nmuc
                  do is=1,2
                     sigm=1.
                     if(is.eq.2) sigm=-1.
ckg                  write(*,*) iv,ipf,imu,is ! compute tau_bounce for distrfun
c                           call xaverage(hhh1,beqv,iav,iter,eps,epsp
c     &                       ,fo(1,imtae0,il,ires),imtae,ntae,ntgr,psma
c     &                       ,psmi,qpfi,pmui,r0,rma,rmi,rvpar,rfres(1,1
c     &                       ,ires,il),sigm,symm,t0,tb,vres(ires,il),z0
c     &                       ,xhp,real(wtae),wb_res,0.,wvpr_res)
c 
                     if(iv.gt.360.and.iv.lt.41.and.imu.eq.46.and.is.eq.1 ! isi=2 for fplot2 is is=1 w/BOT
     &                    .and.ipf.eq.ipfmax)then
                        print *,'iv,ipf,imu,is',iv,ipf,imu,is,ipfmax
                        print *, 'Wtng Enter bef call distrfun'
                        read(input_unit, *) ! continue 
                     endif
                     if(iv.gt.360.and.iv.lt.41.and.imu.eq.45.and.is.eq.1 ! isi=2 for fplot2 is is=1 not BOT
     &                    .and.ipf.eq.ipfmax)then
                        print *,'iv,ipf,imu,is',iv,ipf,imu,is,ipfmax
                        print *, 'Wtng Enter bef call distrfun'
                        read(input_unit, *) ! continue 
     ...
     &                    psiav(iv,imu,ipf,is).gt.0.)then
                        call distrfun(ai,b0*b0oo,dfal,dfalpdv(1,1)
     &                  ,dfalpdpsi,dpga,eps,psiav(iv,imu,ipf,is),gam,
     &                  hhh1,gbisch0(1),ihsps,
...
c     kg                  write(*,*) 'grprpl',iv,ipf,imu,is,v(iv)
                        if(ipf.eq.100.and.iv.eq.29.and.
     &                       imu.gt.90.and.imu.lt.105) then
                           write(*,*) 'grprpl',is,v(iv),
     &              psiav(iv,imu,ipf,is),dfal,dfalpdv(1,1),dfalpdpsi,
     &       tbres_sat(ipf,imu,il,ires,is),vres_sat(ipf,imu,il,ires,is)
                        endif
c     
                        fplot1(imu,iv,is)=dfal+fplot1(imu,iv,is)
                        if(iv.eq.ivc) fplot(ipf,imu,is)=dfal
                    if(is.eq.2.and.iv.eq.ivc) fplot(ipf,imu,1)=(fplot
     &                       (ipf,imu,1)+fplot(ipf,imu,2))*tb
...
c following is not right since psiav is used but it is from NOVA-K grid!
c                           gc2(imui,isi,iv)=(1.-float(imu-1)/nmuc)*sigm
c                           pmu_x=(1.-gc2(imui,isi,iv)**2)*ratmax
c                           qpf_x=psii-sigm*xfow*xhp*v(iv)*sqrt(r0* ! -sqrt(r0*abs(-pmu(1)+r0))*xfow*xhp*v(nv)
c     &                          abs(pmu_x-r0))*bif
c                       call distrfun(ai,b0*b0oo,dfal,dfalpdv,dfalpdpsi,
c     &                          dpga,eps,psiav(iv,imu,ipf,is),gam,hhh1,
c     &                          gbisch0(1),ihsps,
c     &                    iter,qpf_x,p0ga,pmu_x,rvpar,sigm,tb,tip,
c     &                    v(iv),vi,v(nv),0.,z0,xhp,zi,dwbdpphi,wb_res,
c     &                          istart)
                           gc2(imui,isi,iv)=sqrt(abs(1.-pmu(imu)*bii/   ! since psiav is from grid, m.b. 
     &                          (r0*b0)))*sigm                          ! this expression ONLY
                           fplot2(imui,isi,iv)=dfal     ! this is taken at ipfmax 
c                           if(iv.eq.35) then
c                              print *,isi,imu,imui,gc2(imui,isi,iv),
c     & qpf_x,pmu_x,'df',dfal,psiav(iv,imu,ipf,is)
c                           endif
                        endif
...
c     &                       ((qpf(ipf)-psiav(iv,imu,ipf,is)))**2               ! what is missing here is /chi ???
         ...
         continue
         ipfmax=ipfmax
         write(18,*) npfc,nmuc,nv,2
         write(18,*) r0,v(ivc),v(nv),vi,ratmax,psii,valfv  !vi,valfv are in 10^9cm/sec
         write(18,*) qpf
         write(18,*) pmu                                   ! mu*B0/E0*R0=nu (as per Putvinskij notations in his Theory of Plasma Phys. v.18)
         write(18,*) v
         write(18,*) fplot   !(ipf,imu,is)
         write(18,*) fplot1  !(imu,iv,is)
c output v||/v for the distribution function (next line) taken at the LFS point 
         write(18,*) gc2     ! (imui,isi,nv)
         write(18,*) fplot2  ! (imui,is,iv)
         close(18)
cnng14 we dump the coarse DF for further use in the RBQ code. Note that
...
cnng14 dump the DF array
     ...
     &        trim(adjustl(runid(it:90))),';'
         write(18,*) '12'
         write(18,*) 1,8,
     &        '; qpf; pmu; v; conde; condi; dence; dencD; dencT;'
         write(18,*) npfc,' ; qpf;'
         write(18,*) (qpf(i),i=1,npfc)
         write(18,*) nmuc,' ; pmu;'
         write(18,*) (pmu(i),i=1,nmuc)
         write(18,*) nv,' ; v; pk=',pk
         write(18,*) (v(i),i=1,nv)
         write(18,*) nosurf,' ; conde;'
         write(18,*) (condec(i),i=1,nosurf)
         write(18,*) nosurf,' ; condi;'
         write(18,*) (condic(i),i=1,nosurf)
         write(18,*) nosurf,' ; dence;'
         write(18,*) (denc(i,1),i=1,nosurf)
         write(18,*) nosurf,' ; dencD;'
         write(18,*) (denc(i,2),i=1,nosurf)
         write(18,*) nosurf,' ; dencT;'
         write(18,*) (denc(i,3),i=1,nosurf)
         write(18,*) '4 4 ; df_coarse; wbounce; psiav; nusct; '                          ! 4 dmsns & 4 N of arrays
         write(18,*) npfc*nmuc*nv*2,'; df_coarse [cm^-3];'
         write(18,*) ((((f_coarse(iv,imu,ipf,is),iv=1,nv),imu=1,nmuc)
     &        ,ipf=1,npfc),is=1,2)
c         write(18,*) npfc*nmu*nv*2,'; phidot|| [10^6 sec^-1];'
c         write(18,*) ((((wqvpr(iv,imu,ipf,is+wd(iv,imu,ipf,is)),iv=1,nv)
c     &        ,imu=1,nmu),ipf=1,npfc),is=1,2)
         write(18,*) npfc*nmuc*nv*2,'; wbounce [10^6 sec^-1];'
         write(18,*) ((((wb(iv,imu,ipf,is),iv=1,nv),imu=1,nmuc)
     &        ,ipf=1,npfc),is=1,2)
         write(18,*) npfc*nmuc*nv*2,'; psiav ;'
         write(18,*) ((((psiav(iv,imu,ipf,is),iv=1,nv),imu=1,nmuc)
     &        ,ipf=1,npfc),is=1,2)
         write(18,*) npfc*nmuc*nv*2,'; nusct sec^-1 * (pfi / chi)^2;'
         write(18,*) ((((hnub_coarse(iv,imu,ipf,is),iv=1,nv),imu=1,nmuc)
     &        ,ipf=1,npfc),is=1,2)
cnng15  for RBQ we also need more information, such as drift frequencies going into
...
c
c passing particles only without FLR and cutoff
      call twodgraf(qpf,gamintpfi(1,1,1),npfc,1,col(1),pat(1),'white'
     $     ,2,'Passing_w/o_cutoff','sqrt(P_phi)'
     &     ,'d(gam/om)/dsqrt(P_phi)','FOW',6,6,1,wd,0)
      call frame(0)
c trapped particles only without FLR and cutoff
      call twodgraf(qpf,gtrap(1,1),npfc,1,col(1),pat(1),'white'
