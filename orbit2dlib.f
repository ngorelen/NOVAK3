...
cnng22      WRITE (NOUT,99999) SRNAME, ERROR
...
c*********************************************************************72
c
cc GAMAIN computes the incomplete gamma ratio.
c
c  Discussion:
c
...
c
c    Output, double precision GAMAIN, the value of the incomplete 
c    gamma ratio.
c
      implicit none
...
c************************************************************************
C  Bump on tail distribution
C  Dr.Wei Zhang  2023/7/20 IPP, Chinese AS
c here all the velocities are normalized to temperature of ions
c see file BOTfeature.dvi written by N.N.Gorelenkov
c main routine drive for this is main.f file and x is v/v_t
c the rest is explained in the *.dvi file, y= d f/d v, df=f
c it is called from distrfun subroutine
      subroutine BUMP_ON_TAIL(x,y,df,n,cbot,delta,x0)
      IMPLICIT NONE
      integer, parameter :: rk = kind ( 1.0D+00 )
      real(rk) :: cbot, delta, x0
      integer :: n
      real(rk),dimension(n) :: x
      real(rk) :: y(n), II, diff(n), df(n)
      REAL(rk),parameter :: pi=atan(1.D0)*4.D0
      real ( kind = rk ) alngam
      II = - 4./sqrt(pi)*(2*alngam(0.5D0,int((x0/delta)**2))*cbot* ! o11 or I or Factor in my notes
     .             delta*x0**2 - 4*sqrt(pi)*cbot*delta*x0**2 
     .             - 4*alngam(1.D0,int((x0/delta)**2))*cbot*delta**2*x0
     .                   + 2*alngam(1.5D0,int((x0/delta)**2))*cbot*
     .                         -delta**3 2*sqrt(pi)*cbot*delta**3
     .                         - sqrt(pi))/4.
      II = 1./II               
      diff = (-(2*cbot*(x-x0)*exp(-((x-x0)/delta)**2))/delta**2 ! o13 or diff= d f/d v/v_t 
     .      - 2*x*exp(-x**2))*II
      df = ((cbot*exp(-((x-x0)/delta)**2) ! df itself
     .      + exp(-x**2)))*II
      y = diff
      end
c************************************************************************
